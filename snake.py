...
class Snake(Entity):
    def __init__(self):
        ...
        map2d_back = Sprite(texture="Assets/images/2dmap_back.png", scale=0.02, position=(0.76, 0.377),
                            world_parent=camera.ui)
        # On obtient la position de la pomme (entre -13 et 13) dans le jeu et on opere des calculs pour quelle apparaisse au bon endroit sur la carte 2d
        self.pomme2d_x = (
            (self.pomme.position[0] - (-13)) / (13 - (-13))) * (0.836 - 0.68) + 0.68
        self.pomme2d_y = (
            (self.pomme.position[2] - (-13)) / (13 - (-13))) * (0.446 - 0.297) + 0.297
        self.pomme2d = Sprite(texture="Assets/images/apple_pixel.png", scale=0.03, position=(self.pomme2d_x, self.pomme2d_y),
                              world_parent=camera.ui)
        # On obtient la position du joueur (entre -13 et 13) dans le jeu et on opere des calculs pour qu'il apparaisse au bon endroit sur la carte 2d
        self.serpent2d_x = (
            (self.player.position[0] - (-13)) / (13 - (-13))) * (0.836 - 0.68) + 0.68
        self.serpent2d_y = (
            ((self.player.position[2]+1.5) - (-13)) / (13 - (-13))) * (0.446 - 0.297) + 0.297
        self.serpent2d = Sprite(texture="Assets/images/bluesnake2d1.png", scale=0.14, position=(self.serpent2d_x, self.serpent2d_y),
                                world_parent=camera.ui)
        self.test_z = 0
    ...
    def nouvelle_sphere(self):
        ...
        """
        # ajoute une sphere qui apparait a la position de la sphere précédente
    ...
    def enlever_smoothfollow(self):
        """
        fonction qui enlève la fonction SmoothFollow() du script de chaque sphere de maniere à pauser le jeu.
        """
        for j in range(len(self.smoothfollow)):
            self.corps[j+2].scripts.remove(self.smoothfollow[j])
        self.smoothfollow = []

    def activer_smoothfollow(self):
        """
        fonction qui ajoute la fonction SmoothFollow() au script de chaque sphere.
        """
        for j in range(len(self.corps)-2):
            # vérifie si l'entité a déja un script smoothfollow
            if len(self.corps[j+2].scripts) == 0:
                sf = self.corps[j+2].add_script(SmoothFollow(
                    target=self.corps[j+1], offset=[0, 0, 0], speed=12))
                self.smoothfollow.append(sf)  # ajoute le script smoothfollw

    # Pommes

    def spawn_pomme(self):
        """
        fonction qui va faire apparaitre une pomme à une position au hasard de la map.
        """
        # Position au hasasard de la map
        ...
        # On detruit la pomme sur la carte 2d qui vient d'être mangée
        destroy(self.pomme2d)
        self.pomme2d_x = (
            (self.pomme.position[0] - (-13)) / (13 - (-13))) * (0.836 - 0.68) + 0.68
        self.pomme2d_y = (
            (self.pomme.position[2] - (-13)) / (13 - (-13))) * (0.446 - 0.297) + 0.297
        self.pomme2d = Sprite(texture="Assets/images/apple_pixel.png", scale=0.03, position=(self.pomme2d_x, self.pomme2d_y),
                              world_parent=camera.ui)  # On en fait apparaitre une nouvelle aux bonnes coordonnées
    ...
    def intersection_pomme(self):
        ...
        """
        if self.head.intersects(self.pomme):  # Vérifie si la tête du serpent intersecte la pomme
            self.bruit_croc.play()
            self.pomme.disable()
            self.spawn_pomme()
            self.score += 1
            destroy(self.pomme_score)
            self.pomme_score = Text(text=f"{self.score}", position=(-0.69, 0.42), origin=(0, 0),  # Modifie le score
                                    scale=1.5, color=color.yellow)
            # Ajoute quatres spheres a chaque fois que une pomme est mangé
            for i in range(4):
                self.nouvelle_sphere()  # Ajoute 4 nouvelle spheres a la queue du serpent
            self.activer_smoothfollow()
            if len(self.highscores) > 0:
    ...
    def intersection_mur(self):
        ...
        if (self.head.intersects(self.map.mur_avant) or self.head.intersects(self.map.mur_droit) or self.head.intersects(self.map.mur_gauche) or self.head.intersects(self.map.mur_arriere)) and self.interaction_corps == False:
            ...
            self.player.stop()
            self.son_mort.play()
            self.enlever_smoothfollow()
            self.highscores.append(self.score)
            self.highscore_actuel = max(self.highscores)
            self.menu.menu_fin.enabled = True  # Active le menu de fin
    ...
    def intersection_corps(self):
        ...
        if len(self.corps) > 12:
            # Verifie si les parties du corps commençant à l'index 5
    ...
    def update(self):
        ...
        nous l'utilisons pour mettre a jour la position du serpent sur la map 2d et vérifier a chaque frame si la tête du 
        serpent a eu une collision avec une pomme ou un mur ou son propre corps et execute leur code.
        """
        if self.player.always_move_forward == True:
            destroy(self.serpent2d)
            self.serpent2d_x = (
                (self.player.position[0] - (-13)) / (13 - (-13))) * (0.836 - 0.68) + 0.68
            self.serpent2d_y = (
                ((self.player.position[2]+1.5) - (-13)) / (13 - (-13))) * (0.446 - 0.297) + 0.297
            self.serpent2d = Sprite(texture="Assets/images/bluesnake2d1.png", scale=0.14, position=(self.serpent2d_x, self.serpent2d_y),
                    rotation = (0,0,self.player.rotation[1]), world_parent=camera.ui)
            #Prend la rotation du joueur pour faire la même sur la map
    ...
    def reset(self):
        ...
        self.smoothfollow = []
        self.corps = [self.head, Entity(parent=self.head, model="sphere", collider="sphere", texture="Assets/images/tail.jpg", z=-0.5,
                                        scale=0.8, shader=lit_with_shadows_shader)]  # On reinitialise la liste du corps
        self.num_corps = 1
        self.highscores.append(self.score)
        self.score = 0
        destroy(self.pomme_score)
        ...
        destroy(self.pomme)
        for i in range(10):  # Réinitialise les 10 spheres du début
            self.nouvelle_sphere()
            self.corps[i+2].position = (0, -0.4, -12.5-i/1.3)
        self.activer_smoothfollow()
        self.enlever_smoothfollow()
    ...
    def input(self, key):
        """
        touche --> action
        fonction  qui regarde les touches utilisées et leur associe des actions
        ici, elle execute une fonction si une certaine touche est appuyée ou lorsque une touche est selectionnée. 
        """
